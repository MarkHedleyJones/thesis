...
\section{Double Layers}
  ...
  \subsection{Formation}
    ...
    \Cref{fig:butterfat} shows the structure of a butterfat molecule; a long structure that can be approximated as being a solid.
  ...
  \subsection{Structure}
    ...
    \end{figure}
    \Cref{fig:doubleLayer_anatomy} shows a double layer's organisation according to the currently accepted Goüy-Chapmam-Stern model.
...
\section{Impedance Modelling}
  ...
  % \frac{d}{dx}x = 1
  % \end{equation}
  % when the differention operation is only half applied results in \cite{Torokhti1974}:
  % \begin{equation}
  ...
  \subsection{Literature review}
    ...
    Each modifies that model slightly to account for conditions measured outside of its initial predictive capability.
    ...
    A resistor in parallel with the Warburg element is a common method of accounting for Faradaic current.
    ...
    \end{equation}
    % = Y_0 \omega^n\cos (n\pi/2) + j Y_0 \omega^n \sin(n\pi/2)
    ...
    It is unique within the literature for its use of a memristor and diode pair as a means of describing Faradaic current conduction, something thus far described by resistance.
    The model is suitable for entry into commonly used circuit simulators such as SPICE because it is comprised of discrete electronic components, meaning you can build it on a circuit board.

    No comparisons between saline solutions and actual biological fluid, such as cerebrospinal fluid have been found.
