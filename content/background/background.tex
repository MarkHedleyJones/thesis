...
\section{Impedance Modelling}
  ...
  The problem when modelling double layers is that they are modelled by fractional-order functions.
  Fractional order functions can be differentiation or integral functions but are only partially applied.
  For example the equation:
  \begin{equation}
  \frac{d}{dx}x = 1
  \end{equation}
  when only half applied looks like:
  \begin{equation}
  \frac{d^{1/2}}{dx^{1/2}}x = \frac{2\sqrt{x}}{\sqrt{\pi}}
  \end{equation}
  Fractional calculus is not mainstream mathematics but does find use when describing double layer behavior.
  % TODO: Strengthen link between double layer modelling and whatever
  ...
  Their engineers have designed a stimulator fit for human implantation, but there are questions around the electrical impedance their implants will experience once implanted.
  Getting the value wrong means the implant would fail to deliver the desired stimulus current, or otherwise fail due the the presence of an unexpected load.
  Naturally, Saluda want a way to determine that impedance before they put stimulators into into human spinal cavities.
