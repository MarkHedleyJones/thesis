...
\section{Methods Of Parameter Extraction}
  ...
  \subsection{Constant phase element and series resistance}
    ...
    % TODO:
    % Update this section with the list of parameters that will be determined.
    % Mention that the density/number of branches was previously determined and that will be reused.
  ...
  \subsection{Faradaic currents}
    ...
    It is assumed that the fractional differentiation of the current equation will still yield a reletively constant current in light of the capacitor being a CPE.
    So by slowly ramping the electrode overpotential up the current should be roughly constant up to a point where it becomes exponential.
    The voltage corresponding to the point at which the current draw becomes exponential determines the onset of Faradaic conduction.
    That point, together with the rate of growth, are then used to fit values for $i_0$ and $n$, the diode's saturation current and ideality factor respectively.

    % TODO:
    % Discuss Jonathan and Peters method but don't get into details about my method.
    % Perhaps hint that I developed a better method, but use the next section to explain how and why that method is better.
    % Check if I can link that new method somewhere at the start to prop up my contribution to the model.
