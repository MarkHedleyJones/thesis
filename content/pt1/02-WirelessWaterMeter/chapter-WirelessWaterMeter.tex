...
\section{Current Trends in Water Metering}
  ...
  The incentive is that a portion of the household's rates are adjusted according to the volume consumed.
  In these locations, a meter readers visually inspect the displays of each meter; a long and laborious task.
  ...
  an external reader/transmitter that attaches to a compatible meter, or meters built specifically for the purpose.
  ...
  It is estimated that as much as \SI{10}{\percent} of post-meter water consumption is due to leakage in the residential sector.
  Britton et al.\ show that communicating with customers whose homes showed signs of leakage, a night time flow reduction of \SI{89}{\percent} was achieved.
  In contrast, a control group's night-time usage increased by over \SI{50}{\percent}.
  ...
  \section{Envisioned harvester design}
    ...
    A orifice plate with a hole the size of main pipe would cause no pressure across the harvester since it causes no flow obstruction.
  ...
  \section{Quantifying harvestable energy} 
    ...
    In this section the amount of energy available to an energy harvester in a domestic feed is estimated.
    Using a bypass pipe with an orifice plate the pressure drop across the harvester can be controlled.
    Using this pressure control mechanism the pressure drop can be set to match that of a mechanical meter.
    The amount of harvestable energy therefore is the amount of energy dissipated inside a mechanical meter.
    ...
    Heinrich published a similar report in 2007 that contained water flow profiles, these have are considered in the profile~\cite{Heinrich2007}.
    ...
    Volumes for each of the events, and flow profile of the toilet, match the data recorded by Heinrich.
    Specifically these are, \SI{122}{\litre}, \SI{49.5}{\litre} and \SI{6.22}{\litre} for the washing machine, shower and toilet respectively.
    ...
    The following equation fits the trace for the PSM \SI{25}{\milli\meter} meter:
    \begin{equation}
    ...
    \end{figure}
    That equation is plotted in \cref{fig:headloss_fit} on linear scales, adjusted for reasonable flow rates.
