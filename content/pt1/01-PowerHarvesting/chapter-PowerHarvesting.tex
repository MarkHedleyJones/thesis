...
\section{General Analysis}
  ...
  \subsection{Electrical model}
    ...
    \subsubsection*{Optimising $R_{out}$ for maximum power}
      ...
      \Cref{fig:StreamingCell_Schematic-representation} shows that an electrical load placed across a streaming cell is actually placed in parallel with the cell's own internal resistance.
      ...
      % The resistor divider equation for current in parallel branches is:
      % \begin{align}
      %     I_{out} & = I_{source}\times\frac{R_{source}}{R_{source}+R_{load}}
      %     \label{eq:DeterminingOutputPower_extended}
      % \end{align}
    ...
    \subsubsection*{Optimising streaming cell parameters}
      ...
      % We now use the optimum values of $R_{out}$ and $R_{cell}$ to calculate the maximum available power.
      ...
      % \end{align}
      where $P_{max}$ refers to the maximum output power, $\varepsilon=\varepsilon_{0}\,\varepsilon_{r}$ and $R=R_{cell}=R_{out}$.
      ...
      We can identify the presence of $R_{h}$ in \cref{eq:streamingCell_maxPower_substituted}, knowing that the cross-sectional area of the cell ($A$) is its width ($W$) times its height ($\delta$).
...
\section{Results}
  ...
  \subsection{Output power versus load resistance}
    ...
    It was chosen as it was the highest performing cell, in terms of voltage/pressure gradient, that had not yet ruptured.
    Pressure fluctuations near the end of the experiment are due to usage of the department's water system (e.g., a tap being turned on).
